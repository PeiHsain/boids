...
void Pvector::subTwoVector(const Pvector& v, const Pvector& v2)
{
    Pvector tmp(v.x - v2.x, v.y - v2.y);
    // cout << "tmpx : " << tmp.x << "tmpy : " << tmp.y << endl;
    set(tmp.x, tmp.y);
}
...
void Pvector::limit(double max)
{
    ...
    if (size > max) {
        set((x*max/size), (y*max/size));
    }
}

void Pvector::v_tf(double max, double range)
{
    set((x*max/range), (y*max/range));
}
